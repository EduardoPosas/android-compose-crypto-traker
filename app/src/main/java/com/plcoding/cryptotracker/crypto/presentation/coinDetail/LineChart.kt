...
fun LineChart(
    ...
    val selectedDatePointIndex = remember(selectedDataPoint) {
    ...
    Canvas(
        ...
        // Get xLabel measures results, consider multi line
        ...
        val viewPortHeightPx =
            size.height - (maxXLabelHeight + (2 * verticalPaddingPx) + xLabelLineHeight + xAxisLabelSpacing)
...
//        )
        ...
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if (showHelperLines) {
                drawLine(
                    color = if (selectedDatePointIndex == index) style.selectedColor else style.unselectedColor,
                    start = Offset(
                    ...
                    strokeWidth = if (selectedDatePointIndex == index) style.helperLinesThicknessPx * 1.8f else style.helperLinesThicknessPx
                )
            }
            if (selectedDatePointIndex == index) {
                ...
                val textPositionX = if (selectedDatePointIndex == visibleDataPointsIndices.last) {
        ...
        drawPoints = visibleDataPointsIndices.map { index ->
            val x = viewPortLeftX + ((index - visibleDataPointsIndices.first) *
        ...
        // Reference mid connection points for path bezier
